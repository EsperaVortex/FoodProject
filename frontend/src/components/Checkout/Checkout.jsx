...
const Input = ({ label, type = "text", name, value, onChange }) => (
  <div>
    <label className="block mb-1 text-sm font-medium text-amber-200">
      {label}
    </label>
    <input
      type={type}
  ...
  useEffect(() => {
    ...
    if (paymentStatus === "success" && sessionId) {
      axios
        ...
        .catch((err) => setError("Payment confirmation failed"))
      ...
      const payload = {
        ...
        items: cartItems.map((ci) => ({
          item: {
            ...
            imageUrl: ci.item?.imageUrl || "", // Now nested inside `item`
          },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#1a1212] to-[#2a1e1e] text-white py-16 px-4">
      <div className="mx-auto max-w-4xl">
        ...
        {loading && (
          <div className="text-yellow-400 mb-4 text-center">Processing...</div>
        )}
          ...
          <div className="bg-[#4b3b3b]/80 p-6 rounded-3xl space-y-6">
            ...
            <div className="space-y-4 mb-6">
              ...
              {cartItems.map(({ item, quantity }, index) => (
                <div
                  key={item?._id || index}
                  className="flex justify-between items-center text-sm border-b border-amber-300 pb-2"
                >
                  <span>
                    {item?.name || "Unknown Item"} × {quantity || 1}
                  </span>
                  <span>
                    Rs. {((item?.price || 0) * (quantity || 1)).toFixed(2)}
                  </span>
                </div>
              ))}
