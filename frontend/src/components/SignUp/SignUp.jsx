...
import { FaCheckCircle, FaEye, FaEyeSlash, FaArrowLeft, FaExclamationCircle } from 'react-icons/fa'
...
const AwesomeToast = ({ message, icon }) => {
    return (
        ...
        px-6 py-4 rounded-lg shadow-lg border-2 border-amber-300/20'>
            ...
            <span className='font-semibold mr-3 text-[#2D1B0E]'>{message}</span>
        </div>
    )
}
...
const SignUp = () => {
    ...
    useEffect(() => {
        if (showToast.visible && showToast.message === 'Sign Up Successful!') {
            const timer = setTimeout(() => {
                setShowToast({
                    visible: false, // Hide toast before navigation
                    message: '',
                    icon: null
                })
                navigate('/login')
    ...
    const handleSubmit = async e => {
        ...
        try {
            ...
            setShowToast({
                visible: true, // Fixed: should be true to show error
                message: msg,
    ...
    return (
        <div className='min-h-screen flex items-center justify-center bg-[#1a120b] p-4'>
            {/* Fixed: use showToast.visible and dynamic message/icon */}
            ...
            border-4 border-amber-700/30 transform transition-all duration-300 hover:shadow-2xl'>
                ...
                <form onSubmit={handleSubmit} className='space-y-4'>
                    <input type="text" name="username" placeholder='Username' value={formData.username}
                        ...
                        focus:outline-none focus:ring-2 focus:ring-amber-600 transition-all duration-200 
                        hover:scale-[1.02] 'required />
                    ...
                    <div className='relative'>
                        ...
                        <button className='absolute inset-y-0 right-4 flex items-center text-amber-400 hover:text-amber-600
                         transition-transform hover:scale-125' type='button' onClick={toggleShowPassword}>
                            {showPassword ? <FaEyeSlash /> : <FaEye />}
                        </button>
                    </div>
                    <button type='submit' className='w-full py-3 bg-gradient-to-r from-amber-400 to-amber-600
                ...
                <div className='mt-6 text-center'>
                    <Link to='/login' className='group inline-flex items-center text-amber-400 hover:text-amber-600 transition-all
                    duration-300'>
